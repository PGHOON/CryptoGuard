...
void generateRandomFiles(char *files[], int num_files, char *all_files[], int total_files) {
    ...
    for (int i = 0; i < num_files; i++) {
        int index;
        do {
            index = rand() % total_files;
        } while (selected[index]);
        selected[index] = 1;
        files[i] = all_files[index];
    }
}
...
int main() {
    ...
    int total_files = 0;
    collectAllFiles("./dataset/new_0", all_files, &total_files);
    ...
    for (int i = 0; i < TOTAL_SKETCHES; i++) {
        ...
        generateRandomFiles(selected_files, FILES_PER_SKETCH, all_files, total_files);

        processCSVFiles(&cms, selected_files, FILES_PER_SKETCH);

        char filename[256];
        snprintf(filename, sizeof(filename), "./dataset/cms_0/cms_%04d.csv", i);
        saveCMSToCSV(&cms, filename);
        
        #pragma omp critical
        printf("%05d/100000 completed\n", i);
    }
